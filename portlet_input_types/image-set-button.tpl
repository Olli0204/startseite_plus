...
{$useButton   = $propdesc.useButton|default:false}
{$useKat      = $propdesc.useKat|default:false}
{$useKat2     = $propdesc.useKat2|default:false}
{$useLinks2   = $propdesc.useLinks2|default:false}
{$useLinks3   = $propdesc.useLinks3|default:false}

{function slideEntry
    slideData=['desc' => '', 'url' => '', 'link' => '', 'title' => '', 'action' => 'lightbox', 'button' => '', 'kat' => '', 'kat2' => '', 'link2' => '', 'link3' => '']
}
        ...
        <div class="slide-props">
            ...
            {if $useButton}
                <input type="text" class="form-control" placeholder="Button"
                       name="{$propname}[#SORT#][button]" value="{$slideData.button|default:''|escape:'html'}">
            {/if}
            {if $useKat}
                <input type="text" class="form-control" placeholder="Kategorie Links"
                       name="{$propname}[#SORT#][kat]" value="{$slideData.kat|default:''|escape:'html'}">
            {/if}
            {if $useKat2}
                <input type="text" class="form-control" placeholder="Kategorie Rechts"
                       name="{$propname}[#SORT#][kat2]" value="{$slideData.kat2|default:''|escape:'html'}">
            {/if}
            {if $useLinks3}
                <input type="text" class="form-control" placeholder="Link Links"
                       name="{$propname}[#SORT#][link3]" value="{$slideData.link3|default:''|escape:'html'}">
            {/if}
            {if $useLinks2}
                <input type="text" class="form-control" placeholder="Link Rechts"
                       name="{$propname}[#SORT#][link2]" value="{$slideData.link2|default:''|escape:'html'}">
            {/if}
            {if $useLinks}
                <div class="row">
                    <div class="col-4">
